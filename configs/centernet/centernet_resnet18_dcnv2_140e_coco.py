...
model = dict(
    ...
    train_cfg=None,
    test_cfg=dict(topk=100, local_maximum_kernel=3, max_per_img=100, score_thr=0.3))
...
lr_config = dict(
    policy='step',
    warmup='linear',
    warmup_iters=6000,  # same as burn-in in darknet
    warmup_ratio=1.0 / 3,
    step=[50, 60])  # the real step is [18*5, 24*5]
runner = dict(max_epochs=60)  # the real epoch is 28*5=140
